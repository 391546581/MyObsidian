...
# 去掉 --rm，加上 --name 给它个固定的名字方便找，可以查看日志
podman run --name my-compile-job `
  -v D:\retail_cashier:/workspace `
  -v D:\retail_cashier_cache\android:/opt/android-sdk `
  -v D:\retail_cashier_cache\gradle:/home/flutter/.gradle `
  -w /workspace `
  my-flutter-build:3.19.6 `
  flutter build apk --no-tree-shake-icons --release
...
如果成功/失败停掉了，你可以用 podman ps -a 看到它（状态为 Exited）。
你可以查它的日志：podman logs my-compile-job。
看完了记得手动删掉：podman rm my-compile-job。



podman run --rm -v D:\retail_cashier:/workspace -w /workspace my-flutter-build:3.19.6 /bin/bash -c "flutter pub get && flutter build apk --no-tree-shake-icons --release"